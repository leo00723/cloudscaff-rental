...
export class PdfService {
  ...
  async generateEstimate(estimate: Estimate, company: Company) {
    const platforms = [];
    ...
    const data = {
      ...
      content: [
        ...
        // this.getTerms(estimate.company),
      ],
      styles: stylesCS,
  ...
  private getTerms(company: any) {
    const termsRow = [];
    company.terms.forEach((t) => {
      termsRow.push([
        { text: `${t.number}.`, style: 'h6', alignment: 'left' },
        { text: t.name, style: 'h6', alignment: 'left' },
      ]);
    });
    const terms = [
      {
